import { createCli, handleError, pickOptions, validateHost, validateOptions } from "./_chunks/util-MDbeGwC0.mjs";
...
//#region src/reporters/csvSimple.ts
...
//#endregion
//#region src/reporters/csvExpanded.ts
function reportCSVExpanded(reports, { columns }) {
	const { headers, body } = csvSimpleFormat(reports);
	for (const k of Object.keys(columns)) {
		if (k === "overview") continue;
		if (!reports[0].report.categories.find((category) => category.key === k)) continue;
...
//#endregion
//#region src/reporters/jsonExpanded.ts
...
function extractCategoriesFromRoutes(routes) {
	...
	return Object.keys(categoriesWithAllScores).reduce((prev, key) => {
		const averageScore = Number.parseFloat((categoriesWithAllScores[key].scores.reduce((prev$1, curr) => prev$1 + curr, 0) / categoriesWithAllScores[key].scores.length).toFixed(2));
...
function extractMetricsFromRoutes(routes) {
	...
	return Object.keys(metricsWithAllNumericValues).reduce((prev, key) => {
		const averageNumericValue = Number.parseFloat((metricsWithAllNumericValues[key].numericValues.reduce((prev$1, curr) => prev$1 + curr, 0) / metricsWithAllNumericValues[key].numericValues.length).toFixed(2));
...
//#endregion
//#region src/reporters/jsonSimple.ts
...
//#endregion
//#region src/reporters/lighthouseServer.ts
async function reportLighthouseServer(reports, { lhciBuildToken, lhciHost, lhciAuth }) {
	try {
		...
		for (const report of reports) {
			const lighthouseResult = await fs.readJson(`${report.artifactPath}/lighthouse.json`);
			await api.createRun({
				projectId: project.id,
				buildId: build.id,
...
async function outputReport(reporter, config, payload) {
	if (reporter.startsWith("json")) {
		...
		await fs.writeJson(path, payload, { spaces: 2 });
		return path;
	}
	if (reporter.startsWith("csv")) {
...
async function run() {
	...
	hooks.hook("worker-finished", async () => {
		...
		if (resolvedConfig.ci?.buildStatic) {
			...
			const { runtimeSettings, resolvedConfig: resolvedConfig$1 } = useUnlighthouse();
			...
			const relativeDir = `./${relative(resolvedConfig$1.root, runtimeSettings.generatedClientPath)}`;
