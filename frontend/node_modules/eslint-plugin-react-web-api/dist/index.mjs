...
import { ESLintUtils, AST_NODE_TYPES as AST_NODE_TYPES$1 } from '@typescript-eslint/utils';
...
function getCallKind(node) {
  switch (true) {
    case (node.callee.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("addEventListener", "removeEventListener", "abort"))(node.callee.name)):
      return node.callee.name;
    case (node.callee.type === AST_NODE_TYPES$1.MemberExpression && node.callee.property.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("addEventListener", "removeEventListener", "abort"))(node.callee.property.name)):
...
function getOptions(node, initialScope) {
  ...
  function getPropValue(prop, filter = (a) => true) {
    if (prop?.type !== AST_NODE_TYPES$1.Property) return unit;
    const { value } = prop;
    let v = value;
    switch (value.type) {
      case AST_NODE_TYPES$1.Literal: {
        v = value.value;
        break;
      }
      default: {
  ...
  function getOpts(node2) {
    switch (node2.type) {
      case AST_NODE_TYPES$1.Identifier: {
        ...
        if (variableNode?.type === AST_NODE_TYPES$1.ObjectExpression) {
      ...
      case AST_NODE_TYPES$1.ObjectExpression: {
        ...
        const vSignal = pSignal?.type === AST_NODE_TYPES$1.Property ? getSignalValueExpression(pSignal.value, initialScope) : unit;
...
function create(context) {
  ...
  function isSameObject(a, b) {
    switch (true) {
      case (a.type === AST_NODE_TYPES$1.MemberExpression && b.type === AST_NODE_TYPES$1.MemberExpression):
...
function getCallKind2(node) {
  switch (true) {
    case (node.callee.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("setInterval", "clearInterval"))(node.callee.name)):
      return node.callee.name;
    case (node.callee.type === AST_NODE_TYPES$1.MemberExpression && node.callee.property.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("setInterval", "clearInterval"))(node.callee.property.name)):
      return node.callee.property.name;
    default:
      return "other";
  }
...
function isNewResizeObserver(node) {
  return node?.type === AST_NODE_TYPES$1.NewExpression && node.callee.type === AST_NODE_TYPES$1.Identifier && node.callee.name === "ResizeObserver";
}
function isFromObserver(context, node) {
  switch (true) {
    case node.type === AST_NODE_TYPES$1.Identifier: {
...
function getCallKind3(context, node) {
  switch (true) {
    case (node.callee.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("observe", "unobserve", "disconnect"))(node.callee.name) && isFromObserver(context, node.callee)):
      return node.callee.name;
    case (node.callee.type === AST_NODE_TYPES$1.MemberExpression && node.callee.property.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("observe", "unobserve", "disconnect"))(node.callee.property.name) && isFromObserver(context, node.callee)):
      return node.callee.property.name;
    default:
      return "other";
  }
...
function create3(context) {
  ...
  return {
    ...
    ["CallExpression"](node) {
      if (node.callee.type !== AST_NODE_TYPES$1.MemberExpression) {
        return;
      }
      const fKind = fEntries.findLast((x) => x.kind !== "other")?.kind;
    ...
    ["Program:exit"]() {
      for (const { id, node, phaseNode } of observers) {
        ...
        const isDynamic = (node2) => node2?.type === AST_NODE_TYPES$1.CallExpression || isConditional(node2);
...
function getCallKind4(node) {
  switch (true) {
    case (node.callee.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("setTimeout", "clearTimeout"))(node.callee.name)):
      return node.callee.name;
    case (node.callee.type === AST_NODE_TYPES$1.MemberExpression && node.callee.property.type === AST_NODE_TYPES$1.Identifier && isMatching(P.union("setTimeout", "clearTimeout"))(node.callee.property.name)):
      return node.callee.property.name;
    default:
      return "other";
  }
