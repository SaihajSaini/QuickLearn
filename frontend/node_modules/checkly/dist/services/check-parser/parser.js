...
const PACKAGE_EXTENSION = `${node_path_1.default.sep}package.json`;
...
async function validateEntrypoint(entrypoint) {
    const extension = node_path_1.default.extname(entrypoint);
    if (extension !== '.js' && extension !== '.ts' && extension !== '.mjs') {
        throw new Error(`Unsupported file extension for ${entrypoint}`);
    }
    try {
        const content = await promises_1.default.readFile(entrypoint, { encoding: 'utf-8' });
...
class Parser {
    ...
    async getFilesAndDependencies(playwrightConfig) {
        ...
        for (const file of files) {
            ...
            if (STATIC_FILE_EXTENSION.includes(node_path_1.default.extname(file))) {
    ...
    async getFilesFromPaths(playwrightConfig) {
